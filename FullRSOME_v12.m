...
for run_iter= 10
    for ins_iter= 2:5
        ...
        DiaryName= [MatName, '_FullRSOME_v12_', num2str(detergap),'.txt'];
        diary([pwd, '\Runs_group\', '\ins_', num2str(run_iter), '\FullRSOME_v12\', DiaryName])
        disp(DiaryName);
        ...
        %
        termint_time=400*D;    % On solution tim
        ...
        if termint_time_indc~=2
            ...
            teek=tic;
            try
                [Objective(fuliter).outer(iter_solv_Outer,1), deter_time, solving_func_time, sol_status_out, Objective(fuliter).outer_var(iter_solv_Outer)] = RSOME_func (Delta(1).A, Delta(1).b, Pi(1).A, Pi(1).b, M, N, K, T, V, Q, FixC, C, FilCap, Cpur, Chyg, Cins, Cdis, He, Hf, Lf, Le, alpha, Iful(1:M,1), Iemp(1:V,1), termint_time_func, detergap);  % Alg RSOME
            catch
                warning('Time termination. No Rsome Objval.');
                Objective(fuliter).outer(iter_solv_Outer,1)=inf;
                deter_time= deter_time+termint_time_func;
                solving_func_time= deter_time;
                sol_status_out= 207;
                Objective(fuliter).outer_var(iter_solv_Outer)= -1;
            end            
            Modeling_Time=Modeling_Time+toc(teek);
            solving_deter_time= solving_deter_time +deter_time;
            deter_time= 0;
                        ...
                        for i=indx_calc_delta_outer        % i_slct_delta_outer
                            ...
                            for j=indx_calc_pi_outer
                                ...
                                if indcat_forbiden == 0
                                    ...
                                    teek=tic;
                                    try
                                        [Objective(fuliter).outer(iter_solv_Outer,1), deter_time, solving_func_time, sol_status_out, Objective(fuliter).outer_var(iter_solv_Outer)] = RSOME_func (Delta(i).A, Delta(i).b, Pi(j).A, Pi(j).b, M, N, K, T, V, Q, FixC, C, FilCap, Cpur, Chyg, Cins, Cdis, He, Hf, Lf, Le, alpha, Iful(1:M,1), Iemp(1:V,1), termint_time_func, detergap);  % Alg RSOME
                                    catch
                                        warning('Time termination. No Rsome Objval.');
                                        Objective(fuliter).outer(iter_solv_Outer,1)=inf;
                                        deter_time= deter_time+termint_time_func;
        ...
        %% End of Results
        diary off
        save([pwd, '\Runs_group\', '\ins_', num2str(run_iter), '\FullRSOME_v12\',MatName,'_FullRSOME_v12_',num2str(detergap),'.mat'],'Best','Delta','Pi','Objective','maxin','maxout','PoolscenarioU_delta','PoolscenarioU_pi','sol_status_in','sol_status_out','siz_scenarioU_delta','siz_scenarioU_pi','siz_slct_delta','siz_slct_delta_scenario','siz_slct_pi','siz_slct_pi_scenario','siz_Total_delta','siz_Total_pi','fultime','Modeling_Time','solving_time','solution_time','solving_deter_time','solving_Alg24_time','termint_time_func','termint_time_indc');
        zip([pwd, '\Runs_group\', '\ins_', num2str(run_iter), '\FullRSOME_v12\',MatName,'_FullRSOME_v12_',num2str(detergap)],{[pwd, '\Runs_group\', '\ins_', num2str(run_iter), '\FullRSOME_v12\', MatName,'_FullRSOME_v12_',num2str(detergap),'.mat'],[pwd, '\Runs_group\', '\ins_', num2str(run_iter), '\FullRSOME_v12\',MatName, '_FullRSOME_v12_', num2str(detergap),'.txt']});        
        delete([pwd, '\Runs_group\', '\ins_', num2str(run_iter), '\FullRSOME_v12\',MatName,'_FullRSOME_v12_',num2str(detergap),'.mat']);
        delete([pwd, '\Runs_group\', '\ins_', num2str(run_iter), '\FullRSOME_v12\',MatName,'_FullRSOME_v12_',num2str(detergap),'.txt']);
        close all;
